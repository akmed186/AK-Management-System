            ...
            <div class="bg-white dark:bg-gray-800 p-6 shadow-sm sm:rounded-lg">
                <form method="POST" action="{{ route('rooms.update', $room) }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <select id="property_id" name="property_id" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required>
                            @foreach ($properties as $property)
                                <option value="{{ $property->id }}" @selected(old('property_id', $room->property_id) == $property->id)>{{ $property->property_name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <x-text-input id="room_number" name="room_number" type="text" class="mt-1 block w-full" value="{{ old('room_number', $room->room_number) }}" required autofocus />
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input id="floor" name="floor" type="text" class="mt-1 block w-full" value="{{ old('floor', $room->floor) }}" />
                            <x-input-error :messages="$errors->get('floor')" class="mt-2" />
                        </div>
                        <div>
                            <x-input-label for="size_sqft" value="Size (sqft)" />
                            <x-text-input id="size_sqft" name="size_sqft" type="number" step="0.01" min="0" class="mt-1 block w-full" value="{{ old('size_sqft', $room->size_sqft) }}" />
                    ...
                    <div>
                        ...
                        <x-text-input id="base_rent_amount" name="base_rent_amount" type="number" step="0.01" min="0" class="mt-1 block w-full" value="{{ old('base_rent_amount', $room->base_rent_amount) }}" required />
                    ...
                    <div>
                        ...
                        @php $selectedAmenities = collect(old('amenities', $room->amenities->pluck('id')->all())); @endphp
                        <div class="mt-2 grid grid-cols-2 sm:grid-cols-3 gap-2">
                            @foreach ($amenities as $amenity)
                                <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                                    <input type="checkbox" name="amenities[]" value="{{ $amenity->id }}"
                                        @checked($selectedAmenities->contains($amenity->id))
