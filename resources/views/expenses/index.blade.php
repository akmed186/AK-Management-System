<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            @can('create expenses')
                <a href="{{ route('expenses.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 dark:bg-gray-200 text-white dark:text-gray-800 rounded-md text-sm font-medium hover:bg-gray-700 dark:hover:bg-white">
                    Add Expense
                </a>
            @endcan
        </div>
    ...
    <div class="py-12" x-data="{ selected: [], allIds: @json($expenses->pluck('id')) }">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="flex items-center justify-between gap-4">
                <div class="flex items-center gap-2">
                    ...
                    <input type="text" name="search" value="{{ $search }}" placeholder="Search expenses..."
                    ...
                    @if ($search)
                        <a href="{{ route('expenses.index') }}" class="text-sm text-gray-500 dark:text-gray-400 hover:underline">Clear</a>
                    @endif
                </form>

                    <x-export-menu route="expenses.export" />
                </div>

                @can('delete expenses')
                    <form method="POST" action="{{ route('expenses.bulk-destroy') }}"
                        @submit="if (! confirm(`Delete ${selected.length} selected expense(s)?`)) $event.preventDefault()"
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse ($expenses as $expense)
                            <tr>
                                <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{{ $expenses->firstItem() + $loop->index }}</td>
                                @can('delete expenses')
                                    <td class="px-6 py-4">
                                        <input type="checkbox" value="{{ $expense->id }}" x-model="selected"
                                            class="rounded border-gray-300 dark:border-gray-600 text-indigo-600 focus:ring-indigo-500">
                                    </td>
                                @endcan
                                <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">{{ $expense->property->property_name }}</td>
                                ...
                                <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{{ $expense->expense_date->format('M j, Y') }}</td>
                                <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{{ $expense->recordedBy?->name ?? '—' }}</td>
                                <td class="px-6 py-4 text-right text-sm">
                                    <x-row-actions>
                                        @can('edit expenses')
                                            <x-dropdown-link href="{{ route('expenses.edit', $expense) }}">Edit</x-dropdown-link>
                                        @endcan
                                        @can('delete expenses')
