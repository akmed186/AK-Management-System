    ...
    <div class="py-12">
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 p-6 shadow-sm sm:rounded-lg">
                <form method="POST" action="{{ route('utility-types.store') }}" class="space-y-6">
                    @csrf

                    <div>
                        <x-input-label for="utility_name" value="Utility Name" />
                        <x-text-input id="utility_name" name="utility_name" type="text" class="mt-1 block w-full" value="{{ old('utility_name') }}" required autofocus placeholder="e.g. Electricity, Water, Gas, Internet" />
                        <x-input-error :messages="$errors->get('utility_name')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="unit_of_measure" value="Unit of Measure" />
                        <x-text-input id="unit_of_measure" name="unit_of_measure" type="text" class="mt-1 block w-full" value="{{ old('unit_of_measure') }}" required placeholder="e.g. kWh, Gallons, Flat Rate" />
                        <x-input-error :messages="$errors->get('unit_of_measure')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="rate_per_unit" value="Rate Per Unit" />
                        <x-text-input id="rate_per_unit" name="rate_per_unit" type="number" step="0.0001" min="0" class="mt-1 block w-full" value="{{ old('rate_per_unit') }}" required />
