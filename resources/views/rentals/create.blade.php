    ...
    <div class="py-12">
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 p-6 shadow-sm sm:rounded-lg">
                <form method="POST" action="{{ route('rentals.store') }}" class="space-y-6"
                    x-data="{
                ...
                >
                    ...
                    <div>
                        ...
                        @if ($tenants->isEmpty())
                            <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                                No tenants yet — <a href="{{ route('tenants.create') }}" class="text-indigo-600 dark:text-indigo-400 hover:underline">add one first</a>.
                            </p>
                        @endif
                    </div>
                    ...
                    <div>
                        ...
                        @if ($rooms->isEmpty())
                            <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                                No rooms yet — <a href="{{ route('rooms.create') }}" class="text-indigo-600 dark:text-indigo-400 hover:underline">add one first</a>.
                            </p>
                        @endif
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <x-text-input id="security_deposit" name="security_deposit" type="number" step="0.01" min="0" class="mt-1 block w-full" value="{{ old('security_deposit') }}" />
                    ...
                    <div>
                        ...
                        <select id="lease_status" name="lease_status" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required>
                            @foreach (['active', 'expired', 'terminated'] as $status)
                                <option value="{{ $status }}" @selected(old('lease_status', 'active') === $status)>{{ ucfirst($status) }}</option>
                            @endforeach
                        </select>
                        <x-input-error :messages="$errors->get('lease_status')" class="mt-2" />
                        <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Setting this to Active marks the room Occupied automatically.</p>
                    </div>
