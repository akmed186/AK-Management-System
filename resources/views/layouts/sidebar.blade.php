@php
        ...
        'System' => [
            ...
            [
                ...
                'children' => [
                    ...
                    ['route' => 'permissions.index', 'label' => 'Permissions', 'permission' => 'manage roles', 'icon' => 'M15.75 5.25a3 3 0 0 1 3 3m3 0a6 6 0 0 1-7.029 5.912c-.563-.097-1.159.026-1.563.43L10.5 17.25H8.25v2.25H6v2.25H2.25v-2.818c0-.597.237-1.17.659-1.591l6.499-6.499c.404-.404.527-1 .43-1.563A6 6 0 1 1 21.75 8.25Z'],
    ...
    // A "children" link (e.g. System Settings) is visible if at least one of
    // its child links is visible; its own children list is trimmed down to
    // only the ones the user is allowed to see.
    $filterLink = function ($link) use (&$filterLink, $canSeeLink) {
        if (isset($link['children'])) {
            $children = array_values(array_filter($link['children'], $canSeeLink));
        ...
        return $canSeeLink($link) ? $link : null;
    };

    $visibleGroups = collect($isTenant ? $tenantGroups : $adminGroups)
        ->map(fn ($groupLinks) => array_values(array_filter(array_map($filterLink, $groupLinks))))
    ...
    >
        ...
        <nav class="flex-1 px-3 space-y-0.5">
            ...
            @foreach ($visibleGroups as $groupLabel => $groupLinks)
                @php
                    // A group that collapsed down to a single same-named
                    // dropdown (e.g. "Property Management") already shows
                    // its label on the dropdown row, so skip the header.
                    $isSelfTitledDropdown = count($groupLinks) === 1
                        && isset($groupLinks[0]['children'])
                        && $groupLinks[0]['label'] === $groupLabel;
                @endphp

                @unless ($isSelfTitledDropdown)
                ...
                @foreach ($groupLinks as $link)
                    @if (isset($link['children']))
                        @php
                            $wrapperClass = $isSelfTitledDropdown ? 'pt-4' : '';
                            $childActive = collect($link['children'])->contains(
                                fn ($child) => request()->routeIs(explode('.', $child['route'])[0].'*')
                            );
                        @endphp
                        <div x-data="{ open: {{ $childActive ? 'true' : 'false' }} }" class="{{ $wrapperClass }}">
                            <button
                                type="button"
                                @click="open = ! open"
                                class="w-full flex items-center gap-3 px-3 py-2 rounded-md text-sm font-medium transition
                                    {{ $childActive
                                        ? 'bg-indigo-600 text-white shadow-sm'
                                        : 'text-slate-300 hover:bg-slate-800/70 hover:text-white' }}"
                            >
                                ...
                                <svg
                                    class="w-4 h-4 shrink-0 ml-auto transition-transform duration-150"
                            ...
                            <div x-show="open" x-transition class="mt-0.5 ml-4 pl-3 border-l border-slate-700/50 space-y-0.5">
                                @foreach ($link['children'] as $child)
                                    ...
                                    >
                                        ...
                                        @if ($child['route'] === 'users.index' && $unassignedUserCount > 0)
