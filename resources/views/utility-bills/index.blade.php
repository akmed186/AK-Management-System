<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            @can('create utilities')
                <a href="{{ route('utility-bills.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 dark:bg-gray-200 text-white dark:text-gray-800 rounded-md text-sm font-medium hover:bg-gray-700 dark:hover:bg-white">
                    Add Utility Bill
                </a>
            @endcan
        </div>
    ...
    <div class="py-12" x-data="{ selected: [], allIds: @json($utilityBills->pluck('id')) }">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-4">
            <x-flash-status :status="session('status')" />

            <div class="flex items-center justify-between gap-4">
                <div class="flex items-center gap-2">
                    <form method="GET" action="{{ route('utility-bills.index') }}" class="flex items-center gap-2">
                    <input type="text" name="search" value="{{ $search }}" placeholder="Search utility bills..."
                    ...
                    @if ($search)
                        <a href="{{ route('utility-bills.index') }}" class="text-sm text-gray-500 dark:text-gray-400 hover:underline">Clear</a>
                    @endif
                </form>

                    <x-export-menu route="utility-bills.export" />
                </div>

                @can('delete utilities')
                    <form method="POST" action="{{ route('utility-bills.bulk-destroy') }}"
                        @submit="if (! confirm(`Delete ${selected.length} selected bill(s)?`)) $event.preventDefault()"
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse ($utilityBills as $bill)
                            <tr>
                                <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{{ $utilityBills->firstItem() + $loop->index }}</td>
                                @can('delete utilities')
                                    <td class="px-6 py-4">
                                        <input type="checkbox" value="{{ $bill->id }}" x-model="selected"
                                            class="rounded border-gray-300 dark:border-gray-600 text-indigo-600 focus:ring-indigo-500">
                                    </td>
                                @endcan
                                <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">{{ $bill->room->property->property_name }} — {{ $bill->room->room_number }}</td>
                                <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{{ $bill->utilityType->utility_name }}</td>
                                <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{{ $bill->consumption_units }} {{ $bill->utilityType->unit_of_measure }}</td>
                                <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">${{ number_format($bill->total_amount, 2) }}</td>
                                ...
                                <td class="px-6 py-4 text-sm">
                                    <span @class([
                                        ...
                                        'bg-red-100 text-red-800 dark:bg-red-900/40 dark:text-red-300' => $bill->status === 'unpaid',
                                    ])>
                                        {{ ucfirst($bill->status) }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-right text-sm">
                                    <x-row-actions>
                                        @can('edit utilities')
                                            <x-dropdown-link href="{{ route('utility-bills.edit', $bill) }}">Edit</x-dropdown-link>
                                        @endcan
                                        @can('delete utilities')
                                            <form method="POST" action="{{ route('utility-bills.destroy', $bill) }}" onsubmit="return confirm('Delete this bill?');">
                                                @csrf
                                                @method('DELETE')
